...
public class Aaa {
    ...
    public static void tryAuthentication(ParsedData parsed, DbUserSelecting dbUser) {
        if (!Checking.checkLogin(parsed.getLogin(), dbUser)) {
            log.error("\"" + parsed.getLogin() + "\" is wrong username. Exit code : 1");
            System.exit(1);
        } else if (!Checking.checkPassword(parsed, dbUser)) {
    ...
    public static void tryAuthorization(ParsedData parsed, DbUserSelecting dbUser) {
        if (!Checking.checkRole(parsed, dbUser)) {
            log.error("\"" + parsed.getRole() + "\" is wrong role. Exit code : 3");
            System.exit(3);
        } else if (!Checking.checkRoleAndResource(parsed, dbUser)) {
    ...
    public static void tryAccounting(ParsedData parsed, DbUserSelecting dbUser) {
        ...
        User user = dbUser.getUserByLogin(parsed.getLogin());
        ...
        dbUser.addIntoAccounting(new Accounting(curRole, parsed.getResource(), startDay, endDay, volume));
